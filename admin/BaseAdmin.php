...
abstract class BaseAdmin
{
    ...
    const ACCOUNT_ROLE_DEVELOPER = 2;
    const ACCOUNT_ROLE_PRODUCE = 3;
    const ACCOUNT_ROLE_QA = 4;
    const ACCOUNT_ROLE_OPS = 5;
    const ACCOUNT_ROLE_DISTRIBUTOR = 6;
    
    protected static $arAccountRoles = array(
        self::ACCOUNT_ROLE_ADMIN => 'Admin',
        self::ACCOUNT_ROLE_DEVELOPER => 'Developer',
        self::ACCOUNT_ROLE_PRODUCE => 'Produce',
        self::ACCOUNT_ROLE_QA => 'QA',
        self::ACCOUNT_ROLE_OPS => 'Live OPS',
        self::ACCOUNT_ROLE_DISTRIBUTOR => 'Distributor'
    );

    const ADMIN_AUTHOR_UID = "adam_admin_uid";
    ...
    public static function outputMenu()
    {
        ...
        foreach(self::$menus as $k => $v){
            ...
            if(false !== ($index = array_search($v, self::$module))){
//                if($moduleObj->getBit($index + 1) == 0){
//                    continue;
//                }
            }
            if(isset(Setting\Menu::$adminOnly[$v])){
                if($role != self::ACCOUNT_ROLE_ADMIN){
                    continue;
                }
                if(!isset($_SERVER['REQUEST_SCHEME']) || $_SERVER['REQUEST_SCHEME'] != 'https'){
                    continue;
                }
                if(isset(Setting_Menu::$adminOnly[$v]['env'])){
                    if(!defined('ENV')){
                        continue;
                    }
                    if(!in_array(ENV, Setting_Menu::$adminOnly[$v]['env'])){
                        continue;
                    }
                }
            }
...
//        $versionize = static::getConf('versionize');
//        if($versionize){
//            $classArr = explode('_', strtolower($class), 3);
//            Data_Config_Version::addVersion($classArr[2]);
//        }
...
//        }
        ...
        $log = array(
            'adminId' => 1,//self::getLoginedUserId(),
            'request' => $_SERVER['REQUEST_URI'],
            'parameter' => json_encode($_REQUEST),
            'ip' => $_SERVER['REMOTE_ADDR'],
        );
        //Data_Admin_Log::addData($log);
        $method = $action.'Ajax';
        $ret = '';
    ...
    public static function run($disableMenu = false)
    {
        ...
        //self::setSessionCookie(self::ADMIN_AUTHOR_UID, self::getLoginedAccountInfo());
        //self::checkModule();
