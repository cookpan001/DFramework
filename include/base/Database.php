...
class Database
{
    ...
    public static function getConf($master = false, $type = '')
    {
        ...
        if(isset($dbConfig[$type]['host'])){
            return $dbConfig[$type];
        }
        if($master && isset($dbConfig[$type]['master'])){
            $setting = $dbConfig[$type]['master'];
        }else if(!$master && isset($dbConfig[$type]['slave'])){
            $setting = $dbConfig[$type]['slave'];
        }
        if(!isset($setting)){
            return false;
        }
    ...
    protected static function getConnection($master = false, $dbName = '')
    {
        ...
        self::$connMap[$dbName][$master] = $conf;
        self::$md5s[$dbName][$master] = md5($conf['host'].':'.$conf['port'].':'.$conf['database']);
        return $db;
    }
    /**
    ...
    public static function parse($arr, $conf, $table = '')
    {
        ...
        if(isset($conf['primary']) && isset($arr[$conf['primary']])){
            if(!is_array($arr[$conf['primary']])){
                ...
                return array($serial => $table . sprintf('_%02d', $serial));
            }
            $tmp = array();
            foreach($arr[$conf['primary']] as $val){
                $serial = self::getSerialNum($conf, $val);
                if(!isset($tmp[$serial])){
                    $tmp[$serial] = $table . sprintf('_%02d', $serial);
                }
            }
            return $tmp;
        }
        ...
        return array(1 => $table . sprintf('_%02d', 1));
    }
    /**
     * 获取多表时的序号
     * @param type $conf
