...
class QueueServer extends \DF\Base\Daemon
{
    ...
    {
        $this->conf = \DF\Base\Config::getQueue();
        if(empty($this->conf)){
            die("no config for $type.\n");
        }
        if(!empty($this->conf['batch'])){
            $this->batch = $this->conf['batch'];
        }
        if(!empty($this->conf['timeout'])){
            $this->timeout = $this->conf['timeout'];
        }
        if(!empty($this->conf['sleep_us'])){
            $this->sleep = $this->conf['sleep_us'];
        }
        foreach($this->conf['connection'] as $line){
            if(extension_loaded('redis')){
                $redis = new \Redis();
                $redis->pconnect($line['host'], $line['port'], $this->timeout);
            } else if(class_exists('\Predis\Client')){
                $redis = new \Predis\Client("tcp://{$line['host']}:{$line['port']}");
            }else {
                $redis = new \DF\Sys\RedisClient($line['host'], $line['port'], $this->timeout);
            }
            if(!empty($line['password'])){
                $ret = $redis->auth($line['password']);
                if(!$ret || 'OK' != $ret){
                    $this->error("can not connect to {$line['host']}:{$line['port']}");
                    continue;
                }
            }
